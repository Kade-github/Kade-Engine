...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!stageTesting)
		{
			...
			{
				#if debug
				FlxG.log.warn(["Couldn't load player: " + SONG.player1 + ". Loading default player"]);
				#end
		...
		if (positions != null && !stageTesting)
		{
			for (char => pos in positions)
				for (person in [player, gf, opponent])
		...
		if (FlxG.save.data.colour)
			healthBar.createFilledBar(opponent.barColor, player.barColor);
		else
	...
	override public function update(elapsed:Float)
	{
		...
		{
			FlxG.switchState(new AnimationDebug(player.curCharacter));
			clean();
			PlayState.stageTesting = false;
			FlxG.stage.removeEventListener(KeyboardEvent.KEY_DOWN, handleInput);
			FlxG.stage.removeEventListener(KeyboardEvent.KEY_UP, releaseInput);
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (player.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || PlayStateChangeables.botPlay))
		{
			if (player.animation.curAnim.name.startsWith('sing')
				...
				&& (player.animation.curAnim.curFrame >= 10 || player.animation.curAnim.finished))
				player.dance();
		}
